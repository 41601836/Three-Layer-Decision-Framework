...
def backtest_composite(executor, weights, quantile=0.8):
    """回测综合评分策略"""
    ...
    for date in sorted(df['trade_date'].unique()):
        ...
        if len(day_df) >= 50:
            ...
            selected = select_by_quantile(day_df, '_score', quantile)
...
def main():
    ...
    for quantile in [0.70, 0.75, 0.80, 0.85, 0.90]:
        print(f"\n--- Top {int((1-quantile)*100)}% ({quantile}) ---")
        
        result = backtest_composite(executor, weights_v1_1, quantile)
    ...
    for r in results:
        ...
        print(f"Top-{top_pct}%: 胜率={r['win_rate']:.1%}, 信号={r['signal_per_day']:.1f}/天, 回撤={r['max_drawdown']:.1%}, 收益={r['total_return']:.1%} {status}")

...
def main():
    ...
    if meets_target:
        ...
        if result['win_rate'] < 0.60:
            print("   - 胜率低于60%目标")
        elif result['win_rate'] > 0.70:
            print("   - 胜率高于70%目标（可考虑降低门槛增加信号）")
        if result['signal_per_day'] < 1:
            print("   - 信号数低于1个/天")
        elif result['signal_per_day'] > 2:
            print("   - 信号数高于2个/天")

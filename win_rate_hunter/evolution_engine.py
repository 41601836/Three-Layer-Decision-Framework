...
class EvolutionEngine:
    ...
    def fitness_function(self, result: Dict[str, Any]) -> float:
        """综合适应度函数：考虑胜率和信号数约束"""
    ...
    def select(self, elite_count: int = 5) -> List[Strategy]:
        """选择精英策略（基于综合适应度）"""
        self.population.sort(key=lambda x: self.fitness_function(x[1]), reverse=True)
        elites = [p[0] for p in self.population[:elite_count]]
        return elites
    
    def evolve_single_generation(self, elite_count: int = 5):
        """执行单代进化"""
        ...
        print(f"[GEN] 平均胜率: {avg_win_rate:.1%}, 最佳胜率: {self.best_result['win_rate']:.1%}")

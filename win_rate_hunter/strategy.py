...
def get_dynamic_thresholds(market_phase: str) -> Dict[str, float]:
    """根据市场阶段返回不同阈值"""
    if market_phase == 'early' or market_phase == 'stage_1':
        return {'min_score': 0.4, 'factor_threshold': 0.3}
    elif market_phase == 'mid' or market_phase == 'stage_2':
        return {'min_score': 0.5, 'factor_threshold': 0.5}
    else:
        return {'min_score': 0.6, 'factor_threshold': 0.7}

def get_market_phase(date: pd.Timestamp) -> str:
    """根据日期判断市场阶段"""
    month = date.month
    if 1 <= month <= 2:
        return 'stage_1'
    ...
    def generate_signal(self, df: pd.DataFrame, max_per_day: int = 50) -> pd.Series:
        ...
        if self.dynamic_threshold:
            ...
            for date in df['trade_date'].unique():
                ...
                if len(day_df) > 0:
                    market_phase = get_market_phase(date)
                    thresholds = get_dynamic_thresholds(market_phase)
                    min_score_ratio = thresholds['min_score']
                    
                    day_df = day_df[day_df['_score'] >= total_weight * min_score_ratio]
                    
                    if len(day_df) > 0:
                        top_stocks = day_df.sort_values('_score', ascending=False).head(max_per_day)
                        signal.loc[top_stocks.index] = True
        else:
            df['_signal_raw'] = score >= total_weight * 0.7
            signal = pd.Series(False, index=df.index)

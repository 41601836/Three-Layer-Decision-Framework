...
class BacktestExecutor:
    ...
    def load_data(self) -> pd.DataFrame:
        ...
        # 计算低波排名（越低波动越好，排名越高）
        daily['low_vol_rank'] = daily.groupby('trade_date')['volatility_60d'].rank(pct=True, ascending=True)
        
        # ROE因子（从数据库获取，如果不存在则用PB和PE估算）
        if 'roe' in daily.columns:
            daily['roe'] = daily['roe'] / 100  # 转换为小数
        else:
            # 使用PB和PE估算ROE（简化处理）
            daily['roe'] = daily.apply(lambda row: (1/row['pe']) if row['pe'] > 0 else 0, axis=1)

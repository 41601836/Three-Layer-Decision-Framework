...
from tushare_fund_flow import TushareFundFlow

class TushareEnhancedStrategy:
    def __init__(self):
        self.weights = {
            'pb': 0.8,
            'ret_20d': 1.8,
        ...
        self.hold_days = 10
        self.fund_flow = TushareFundFlow()
    ...
    def backtest(self, executor):
        ...
        if self.fund_flow.pro:
            print("✅ 使用真实Tushare资金流数据")
            codes = df['ts_code'].unique()
            
            for code in codes[:100]:
                mf_data = self.fund_flow.get_stock_moneyflow(code, executor.start_date, executor.end_date)
                if not mf_data.empty:
                    mf_map = mf_data.set_index('trade_date')['net_mf_ratio'].to_dict()
                    mask = (df['ts_code'] == code) & df['trade_date'].isin(mf_map.keys())
                    df.loc[mask, 'net_mf_ratio'] = df.loc[mask, 'trade_date'].map(mf_map)
        else:
            print("⚠️ 使用模拟资金流数据")
            df['net_mf_ratio'] = np.random.uniform(-0.1, 0.1, len(df))
        ...
        return {
            ...
            'using_real_data': self.fund_flow.pro is not None
        }

def main():
    executor = BacktestExecutor("20260101", "20260625")
    strategy = TushareEnhancedStrategy()
    
    print("="*70)
    print("          Tushare资金流增强策略 - 回测结果")
    print("="*70)
    print(f"数据来源: {'✅ 真实Tushare数据' if strategy.fund_flow.pro else '⚠️ 模拟数据'}")
    print()
    print(f"策略参数:")
    ...
    if not strategy.fund_flow.pro:
        print("💡 提示: 配置Tushare Token获取真实资金流数据")
        print("  1. 在Tushare官网获取Token: https://tushare.pro")
        print("  2. 执行: export TUSHARE_TOKEN=your_token")
        print("  3. 重新运行此脚本")
        print()

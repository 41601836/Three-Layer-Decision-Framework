...
def apply_basic_filters(df):
    ...
    filtered = filtered[filtered['pct_chg'].abs() < 10]
    return filtered
...
def backtest_composite(executor, weights, directions, top_n=10, min_score_ratio=0.6):
    """回测综合评分策略"""
...
def main():
    ...
    for weight_set in weights_candidates:
        for top_n in [6, 8, 10]:
            for min_score_ratio in [0.5, 0.55, 0.6]:
                print(f"\n--- {weight_set['name']}, Top-{top_n}, 最小评分{min_score_ratio} ---")
                
                result = backtest_composite(executor, weight_set['weights'], directions, top_n, min_score_ratio)
    ...
    for r in results:
        ...
        print(f"{r['name']}, Top-{r['top_n']}, 评分{r['min_score_ratio']}: 胜率={r['win_rate']:.1%}, 信号={r['signal_per_day']:.1f}/天, 回撤={r['max_drawdown']:.1%}, 收益={r['total_return']:.1%} {status}")

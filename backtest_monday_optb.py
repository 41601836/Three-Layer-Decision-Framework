# -*- coding: utf-8 -*-
"""
周波段·周一战法 v1.2 优化版 (方案 B: 纯日线因子化)
================================================
核心修改：
  1. 抛弃周 MACD，改为日线级别 MACD (daily_macd_ok)。
  2. 保证指标无“周内不完整漂移”影响。
  3. 保留周二开盘建仓、周五强平的战法节奏，进行干净的因子对比。
"""
...
# 日 MACD 计算
# =============================================================================
def compute_daily_macd(df):
    print("[MACD] 向量化计算日 MACD 因子...")
    df = df.sort_values(["ts_code", "trade_date"]).copy()
    
    # 向量化计算日线 EMA 和 MACD
    df["ema12"] = df.groupby("ts_code")["close"].transform(lambda x: x.ewm(span=12, adjust=False).mean())
    df["ema26"] = df.groupby("ts_code")["close"].transform(lambda x: x.ewm(span=26, adjust=False).mean())
    df["dif"] = df["ema12"] - df["ema26"]
    df["dea"] = df.groupby("ts_code")["dif"].transform(lambda x: x.ewm(span=9, adjust=False).mean())
    df["macd_hist"] = (df["dif"] - df["dea"]) * 2
    
    df["prev_macd_hist"] = df.groupby("ts_code")["macd_hist"].shift(1)
    df["daily_macd_ok"] = (df["macd_hist"] > df["prev_macd_hist"]) & (df["dif"] > df["dea"])
    return df
...
def run_monday_strategy_simulation(daily_res, money, trade_dates, conn, start_date):
    ...
    for w_idx, week_info in enumerate(weeks_list):
        ...
        # 因子过滤 (周 MACD 换成日 MACD)
        candidates = mon_df[
            mon_df["industry"].isin(strong_industries) &
            (mon_df["daily_macd_ok"] == True) &
            (mon_df["ma20_ok"] == True) &
...
def analyze_performance(df_equity, df_trades):
    ...
    print(f"\n{'='*70}")
    print("  StockAI v4.0 - 周一战法优化方案B (纯日线因子化) 量化报告")
    print(f"{'='*70}")
    print(f"\n【优化版交易绩效】")
...
def main():
    parser = argparse.ArgumentParser(description="周波段·周一战法优化方案 B 离线回测")
    parser.add_argument("--start",  default="20250101")
    parser.add_argument("--end",    default="20251231")
    parser.add_argument("--output", default="reports/backtest_monday_optb.csv")
    ...
    try:
        ...
        daily_res = compute_daily_macd(daily)

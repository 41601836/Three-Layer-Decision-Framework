...
# ═══════════════════════════════════════════════════════════════════════════════
# 极速模式：按交易日批量抓取（提速 5000x）
# ═══════════════════════════════════════════════════════════════════════════════
def fetch_by_date_main(global_start: str, global_end: str,
                       fetch_money=True, fetch_daily_basic=True,
                       fetch_margin=True, fetch_block=True, fetch_bak=True):
    log.info("⚡ 开启极速天级数据同步模式...")
    start_ts = time.time()
    ...
    try:
        cal_df = pro.trade_cal(exchange="SSE", start_date=global_start, end_date=global_end, is_open="1")
        trade_dates = sorted(cal_df["cal_date"].tolist()) if cal_df is not None and not cal_df.empty else []
        time.sleep(RATE_LIMIT_SLEEP)
    except Exception as e:
        log.error("❌ 无法获取交易日历：%s", e)
        conn.close()
        return

    if not trade_dates:
        log.warning("ℹ️  未找到有效的交易日，可能是日期区间未开市")
        conn.close()
        return

    log.info("📅 本次待检查的交易日：%d 天 (%s ~ %s)", len(trade_dates), trade_dates[0], trade_dates[-1])
    total_new_rows = 0
    
    for idx, date_str in enumerate(trade_dates, 1):
        log.info("-" * 50)
        log.info("👉 [%d/%d] 正在同步交易日: %s ...", idx, len(trade_dates), date_str)
        
        try:
            count = conn.execute("SELECT COUNT(*) FROM daily_prices WHERE trade_date = ?", (date_str,)).fetchone()[0]
        except Exception:
            count = 0
            
        if count > 4000:
            log.info("⏭  交易日 %s 日线数据已存在（%d 行），跳过该日拉取", date_str, count)
            continue
        ...
        # 2. 抓取 daily_basic
        if fetch_daily_basic:
            try:
                log.info("  -> 拉取全市场每日基本指标...")
                df_basic = pro.daily_basic(trade_date=date_str, fields="ts_code,trade_date,turnover_rate,volume_ratio,pe,pb,ps,total_share,float_share,free_share,total_mv,circ_mv")
                time.sleep(RATE_LIMIT_SLEEP)
                if df_basic is not None and not df_basic.empty:
                    rows = [
                        (r.ts_code, r.trade_date,
                         ...
                         _safe(r,'pe'), _safe(r,'pb'), _safe(r,'ps'),
                         _safe(r,'total_share'), _safe(r,'float_share'), _safe(r,'free_share'),
                         _safe(r,'total_mv'), _safe(r,'circ_mv')) 
                        for r in df_basic.itertuples(index=False)
                    ]
                    with DB_LOCK:
                        conn.executemany("INSERT OR REPLACE INTO daily_basic VALUES (?,?,?,?,?,?,?,?,?,?,?,?)", rows)
                        conn.commit()
                    log.info("  ✅ 成功写入 daily_basic: +%d 行", len(rows))
                    total_new_rows += len(rows)
            except Exception as e:
                log.warning("  ⚠️ daily_basic 写入失败: %s", e)

        # 3. 抓取 moneyflow
        if fetch_money:
            try:
                log.info("  -> 拉取全市场主力资金流向...")
                ...
                if df_mf is not None and not df_mf.empty:
                    rows = [
                        (r.ts_code, r.trade_date,
                         ...
                         _safe(r,'sell_elg_vol'), _safe(r,'sell_elg_amount'),
                         _safe(r,'net_mf_vol'),   _safe(r,'net_mf_amount'))
                        for r in df_mf.itertuples(index=False)
                    ]
                    with DB_LOCK:
                        conn.executemany("""
                            INSERT OR REPLACE INTO moneyflow
                        ...
                        """, rows)
                        conn.commit()
                    log.info("  ✅ 成功写入 moneyflow: +%d 行", len(rows))
                    total_new_rows += len(rows)
            except Exception as e:
                log.warning("  ⚠️ moneyflow 写入失败: %s", e)

        # 4. 抓取 margin_detail
        if fetch_margin:
            try:
                log.info("  -> 拉取全市场融资融券明细...")
                df_margin = pro.margin_detail(trade_date=date_str)
                time.sleep(RATE_LIMIT_SLEEP)
                if df_margin is not None and not df_margin.empty:
                    rows = [
                        (r.ts_code, r.trade_date,
                         _safe(r,'rzye'), _safe(r,'rqye'), _safe(r,'rzmre'), _safe(r,'rqyl'),
                         _safe(r,'rzche'), _safe(r,'rqchl'), _safe(r,'rqmcl'), _safe(r,'rzrqye'))
                        for r in df_margin.itertuples(index=False)
                    ]
                    with DB_LOCK:
                        conn.executemany("INSERT OR REPLACE INTO margin_detail VALUES (?,?,?,?,?,?,?,?,?,?)", rows)
                        conn.commit()
                    log.info("  ✅ 成功写入 margin_detail: +%d 行", len(rows))
                    total_new_rows += len(rows)
            except Exception as e:
                log.warning("  ⚠️ margin_detail 写入失败: %s", e)

        # 5. 抓取 block_trade
        if fetch_block:
            try:
                log.info("  -> 拉取全市场大宗交易明细...")
                df_block = pro.block_trade(trade_date=date_str)
                time.sleep(RATE_LIMIT_SLEEP)
                if df_block is not None and not df_block.empty:
                    ...
                    with DB_LOCK:
                        conn.executemany("INSERT OR REPLACE INTO block_trade VALUES (?,?,?,?,?,?,?,?)", rows)
                        conn.commit()
                    log.info("  ✅ 成功写入 block_trade: +%d 行", len(rows))
                    total_new_rows += len(rows)
            except Exception as e:
                log.warning("  ⚠️ block_trade 写入失败: %s", e)

        # 6. 抓取 bak_basic
        if fetch_bak:
            try:
                log.info("  -> 拉取全市场备用基本指标...")
                df_bak = pro.bak_basic(trade_date=date_str)
                time.sleep(RATE_LIMIT_SLEEP)
                if df_bak is not None and not df_bak.empty:
                    df_bak = df_bak.dropna(subset=['trade_date'])
                    rows = [
                        (r.ts_code, r.trade_date,
                         _safe(r,'name'), _safe(r,'industry'), _safe(r,'area'),
                         _safe(r,'pe'), _safe(r,'float_share'), _safe(r,'total_share'),
                         ...
                         _safe(r,'per_undp'), _safe(r,'rev_yoy'), _safe(r,'profit_yoy'),
                         _safe(r,'gpr'), _safe(r,'npr'))
                        for r in df_bak.itertuples(index=False)
                    ]
                    with DB_LOCK:
                        conn.executemany("INSERT OR REPLACE INTO bak_basic VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", rows)
                        conn.commit()
                    log.info("  ✅ 成功写入 bak_basic: +%d 行", len(rows))
                    total_new_rows += len(rows)
            except Exception as e:
                log.warning("  ⚠️ bak_basic 写入失败: %s", e)

    conn.close()
    elapsed = time.time() - start_ts
    log.info("\n🏁 极速同步完成！总计耗时 %.1f 秒 | 新增 %d 行", elapsed, total_new_rows)
...
def parse_args():
    ...
    parser.add_argument("--fast", action="store_true", help="极速按天拉取全市场数据（日度增量同步）")

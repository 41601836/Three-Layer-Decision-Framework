...
def send_market_summary_card(
    ...
    # 格式化百分比函数：A股风格（涨红、跌绿），带符号，保留2位小数
    ...
    card = {
        ...
        "header": {
            "title": {"tag": "plain_text", "content": "📊 StockAI · 市场全局汇总"},
...
def send_sector_summary_card(
    ...
    card = {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": "📈 StockAI · 板块热力图"},
            "subtitle": {"tag": "plain_text", "content": date_str},
            "template": "purple"
        },
        "elements": elements
...
def send_stock_report_v2(
    ...
    for key, value in score_details.items():
        if isinstance(value, (int, float)):
            if value > 0:
                add_items.append(f"- +{value} 分")
            elif value < 0:
                deduct_items.append(f"- {value} 分")
        else:
            if "分" in str(value):
                add_items.append(f"- {value}") if "+" in str(value) else deduct_items.append(f"- {value}")
            else:
                add_items.append(f"- {value}")
    ...
    content_lines.append(f"- **主力资金（万元）**：{main_money_val:+.2f}")
    
    total_vol_val = total_volume if total_volume is not None else 0.0
    content_lines.append(f"- **总成交额（万元）**：{total_vol_val:,.2f}")
    
    ratio = (abs(main_money_val) / total_vol_val * 100) if total_vol_val > 0 else 0.0
    content_lines.append(f"- **主力资金占比**：{ratio:.2f}%")
    
    content_lines.append(f"- **股东户数变化（%）**：{holder_chg_val:+.2f}%")
    
    holder_curr_val = holder_current if holder_current is not None else 0
    holder_prev_val = holder_prev if holder_prev is not None else 0
    content_lines.append(f"- **当前股东户数**：{holder_curr_val:,} 户")
    content_lines.append(f"- **上期股东户数**：{holder_prev_val:,} 户")
    
    content_lines.append(f"- **每股成本**：¥{close_price:.2f}")
    
    hsgt_5d_val = hsgt_5d if hsgt_5d is not None else 0.0
    content_lines.append(f"- **北向资金5日净流入（万元）**：{hsgt_5d_val:+.2f}")
    
    content_lines.append("")
    if downgrade_reason:
        original_total = python_score + ai_score
        content_lines.append(f"**降级原因**：{downgrade_reason}")
        content_lines.append(f"**总分计算公式**：Python评分({python_score:.0f}) + AI评分({ai_score:.0f}) = {original_total:.0f}分 → 最终得分 **{total_score:.0f}分**")
    else:
    ...
    card = {
        ...
        "header": {
            "title": {"tag": "plain_text",
                "content": f"🔥 StockAI 精选金股 · {ts_code} {name}"},
...
def send_daily_summary(
    ...
    card = {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": "🔥 StockAI · 今日精选金股"},
            "subtitle": {"tag": "plain_text", "content": date_str},
            "template": header_color
        },
        "elements": [
...
def send_markdown_card(title: str, content: str, grade: str = "") -> bool:
    ...
    card_elements = []
    content_str = content
    while len(content_str) > 0 and len(card_elements) < 6:
        card_elements.append({"tag": "markdown", "content": content_str[:4000]})
...
def send_daily_brief(
    ...
    card = {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": f"📈 StockAI · 今日操作简报"},
            "subtitle": {"tag": "plain_text", "content": date_str},
            "template": "blue"
        },
        "elements": [
...
def send_full_daily_briefing(
    ...
    card = {
        "config": {"wide_screen_mode": True},
        "header": {
            "title": {"tag": "plain_text", "content": "🔥 StockAI · 飞书推送更新"},
            "subtitle": {"tag": "plain_text", "content": date_str},
...
if __name__ == "__main__":
    ...
    ok1 = send_market_summary_card(
        ...
        ma5_ratio=0.42,
        main_sectors=[
            {"name": "白酒", "current": 1.85, "sum_5": 3.42, "sum_10": 5.11, "sum_20": 8.73},
            {"name": "电力", "current": 1.12, "sum_5": 2.05, "sum_10": 3.88, "sum_20": 6.14},
            {"name": "金融", "current": 0.65, "sum_5": 1.33, "sum_10": 2.44, "sum_20": 3.97},
        ],
        backup_sectors=[
            {"name": "光伏", "current": 0.88, "sum_5": 1.22, "sum_10": 1.95, "sum_20": 2.64},
            {"name": "半导体", "current": 0.45, "sum_5": 0.88, "sum_10": 1.12, "sum_20": 1.88},
        ],
        decline_sectors=[
            {"name": "房地产", "current": -1.23, "sum_5": -2.44, "sum_10": -3.88, "sum_20": -5.11},
            {"name": "煤炭", "current": -0.95, "sum_5": -1.66, "sum_10": -2.55, "sum_20": -3.88},
        ]
    )
